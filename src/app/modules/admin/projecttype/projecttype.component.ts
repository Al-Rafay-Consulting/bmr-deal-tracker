...
export class ProjecttypeComponent extends SpBLBase<Projecttype> implements OnInit  {
  ...
  constructor(
    ...
    public messageDialog?: MessageDialogeService
      ...
      this.formTitle = "Project Type";
  
      //Defined SP List Title
      this.addListTitle("ProjectType", "ProjectType");
      this.query = {
        select: 'ID, SortNo,Title, Modified, Editor/Title,Inactive',
        expand: 'Editor'
      ...
      onCreate(form: NgForm) {
        this.router.navigate(['/admin/Projecttype/form'], { replaceUrl: true });
        //BLBase function
        this.Create();
      }
  ...
  onSubmit(form: NgForm) {
    //BLBase function
    var objProjecttype =  new Projecttype();
    objProjecttype.ID = this.formData.ID;
    objProjecttype.Title = this.formData.Title;
    objProjecttype.SortNo = this.formData.SortNo;
    objProjecttype.Inactive = this.formData.Inactive;
    this.insertRecord(objProjecttype);
  }

  AfterInsert(jsonObject: any) {
    this.router.navigate(['/admin/Projecttype/list']);
  }
